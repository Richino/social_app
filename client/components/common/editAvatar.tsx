	...
	return (
		<div id="popup-profile" className="fixed left-0 top-0 z-50 flex h-full w-full items-center justify-center bg-black/80 p-5" onClick={hidePopup}>
			<div className="flex w-full max-w-[300px]  flex-col rounded-md bg-white font-bold dark:bg-neutral-900">
				...
				<button className="border-b border-neutral-200 py-4 text-red-500 dark:border-neutral-800" onClick={deletePhoto}>
					Delete Photo
				</button>
			...
			<input id="file-input-upload-photo" type="file" name="post" accept="image/png, image/jpeg" className="hidden" onChange={uploadProfileImage} />
		</div>
	);
}
